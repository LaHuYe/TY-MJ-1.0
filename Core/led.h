...
/* ==================== 用户配置区 ==================== */
#define WS2812B_ENABLE 0 // 是否使用WS2812B灯珠
...
#define ALTERNATING_BLINK_ENABLE 0
#define BREATH_ENABLE            0
#define RUNNING_ENABLE           0
#define MARQUEE_ENABLE           1
// 定义LED开关状态
#if (WS2812B_ENABLE)
#define LED_ON  led->ledColor
#define LED_OFF (ws2812b_color_t){0, 0, 0}
#else
#define LED_ON  1
#define LED_OFF 0
#endif
...
typedef enum
{
    ...
    LED_EVENT_STARTUP_DISPLAY,    // 开机显示事件
    LED_EVENT_KEY_OPERATION,      // 按键操作事件
    LED_EVENT_CHARGING,           // 充电状态显示事件
    LED_EVENT_FULL_CHARGING,      // 充电满状态显示事件
    LED_EVENT_LOW_POWER_SHUTDOWN, // 低电关机事件
...
#if (WS2812B_ENABLE)
    ws2812b_color_t ledColor;     // LED颜色RGB
#endif
#if (BLINK_ENABLE)
...
// LED状态设置函数
#if (WS2812B_ENABLE)
typedef void (*LED_SetState_t)(uint16_t led, ws2812b_color_t color);
#else

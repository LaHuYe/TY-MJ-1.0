...
{
    HAL_GPIO_WritePin(ADDR_TX_GPIO_PORT, ADDR_TX_PIN, level);
}
...
{
    // 使能GPIO时钟
    ADDR_TX_GPIO_CLK_ENABLE();

    // 配置GPIO为推挽输出模式
    GPIO_InitTypeDef GPIO_InitStruct = {0};
    GPIO_InitStruct.Pin = ADDR_TX_PIN;
    GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
    GPIO_InitStruct.Pull = GPIO_PULLDOWN;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
    HAL_GPIO_Init(ADDR_TX_GPIO_PORT, &GPIO_InitStruct);

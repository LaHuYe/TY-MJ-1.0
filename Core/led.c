...
static bool LED_ADD_EN = true; // LED事件添加使能

// LED事件添加使能
void LED_EventAddEN(bool enable)
{
    LED_ADD_EN = enable;
}
...
void LED_EventAdd(LED_Event_t ledEvent)
{
    ...
    // 添加新的事件时关闭所有LED
    if (ledEventTable[ledEvent].event != 0)
    {
        LED_SetState(0xFF, LED_OFF);
    }
...
void LED_EventHandle(void)
{
    ...
    // 如果所有LED事件都执行完了，则关闭所有LED
    if (!LED_IsEventActive())
    {
        LED_SetState(0xFF, LED_OFF);
    }
}
...
static uint8_t LED_Blinking_Update(LED_Config_t *led)
{
    ...
    // 计算时间差
    uint32_t diff_time = GetTickDiff(led->currentTime);
    // 计算已经进行了多少次闪烁
    uint16_t blinkCount = diff_time / run_cycle;
    // 计算当前时间在周期中的位置
    uint32_t sec_left = diff_time % run_cycle;
    // 判断是否大于运行次数
    if (led->runNum > 0 && blinkCount >= led->runNum)
    {
        LED_SetState(led->ledMask, LED_OFF); // 关闭LED
...
static uint8_t LED_AlternatingBlinking_Update(LED_Config_t *led)
{
    ...
    uint32_t diff_time = GetTickDiff(led->currentTime);
    // 计算已经进行了多少次交替
    uint16_t blinkCount = diff_time / led->alternatingBlinkParams.Cycle;
    // 计算当前时间在周期中的位置
    uint32_t sec_left = diff_time % led->alternatingBlinkParams.Cycle;
...
static uint8_t LED_Breathing_Update(LED_Config_t *led)
{
    ...
    uint32_t diff_time = GetTickDiff(led->currentTime);
    // 计算已经进行了多少次呼吸
    uint16_t breathCount = diff_time / breathCycleTime;

    // 计算当前时间在周期中的位置
    uint32_t timeInCycle = diff_time % breathCycleTime;

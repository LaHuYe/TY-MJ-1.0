...
/*! ********************************************************
 * @name    CMT2300A_DelayNops
 * @desc    Delayed by NOPS.
 * *********************************************************/
void CMT2300A_DelayNops(uint32_t Nops) // 1ms:	4788
{
...
BOOL CMT2300A_AutoSwitchStatus(u8 nGoCmd)
{
    ...
    while (HAL_GetTick() - nBegTick < 10)
    {
        CMT2300A_DelayNops(478);
...
u8 Radio_Send_FixedLen(const u8 pBuf[], u8 len)
{
    ...
    while (1)
    {
        ...
        if (delay == 0) ////发送超时溢出，防止芯片死机，可根据实际开发情况调整超时时间
        {
            RF_Init();
            return 0; // 发送超时
        }
